...
_PAIRS_TEMPLATE = '''"""Self-contained PAIRS backtest: __A__ vs __B__ since __START__ - generated by the Quant Backtester.
Trades the SPREAD of two assets (dollar-neutral), point-in-time + 1-bar lag.  Run:  python this_file.py"""
import yfinance as yf
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
# === 1. DATA: fetch both legs, align to the common window ===
def fetch(ticker, start):
    df = yf.download(ticker, start=start, auto_adjust=True, progress=False)["Close"]
    if isinstance(df, pd.DataFrame):
        df = df.iloc[:, 0]
...
# === 2. STRATEGY: the coder's spread signal  strategy_pair(history_a, history_b) -> position in [-1, 1] ===
__STRATEGY_CODE__
...
# === 4. RUN + METRICS + PLOT ===
if __name__ == "__main__":
    pa, pb = fetch("__A__", "__START__"), fetch("__B__", "__START__")
    equity, position, pnl = run_pairs_backtest(pa, pb, strategy_pair)
    total = equity.iloc[-1] - 1.0
    sharpe = pnl.mean() / pnl.std() * np.sqrt(252) if pnl.std() > 0 else 0.0
    max_dd = (equity / equity.cummax() - 1.0).min()
    n_trades = int((position.diff().abs() > 0).sum())
    print(f"__A__-__B__  total={total:+.1%}  Sharpe={sharpe:.2f}  maxDD={max_dd:.1%}  trades={n_trades}")

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 9), sharex=True,
                                   gridspec_kw={"height_ratios": [3, 1]})
    ax1.set_title("__A__ - __B__  spread strategy (growth of $1, dollar-neutral)")
    ax1.plot(equity.index, equity, color="blue", lw=1.1, label="Spread equity")
    ax1.legend(); ax1.grid(True, alpha=0.3)
    ax2.fill_between(position.index, position, step="pre", color="purple", alpha=0.3)
    ax2.set_ylabel("Spread position"); ax2.set_ylim(-1.1, 1.1); ax2.grid(True, alpha=0.3)
    plt.tight_layout(); plt.show()
'''


def full_pairs_script(strategy_code, ticker_a="XLF", ticker_b="XLI", start="2019-01-01") -> str:
    """Assemble a standalone pairs backtest (spread engine, point-in-time + lag, dollar-neutral)."""
    return (_PAIRS_TEMPLATE
            .replace("__A__", ticker_a)
            .replace("__B__", ticker_b)
            .replace("__START__", str(start))
            .replace("__STRATEGY_CODE__", strategy_code.strip()))
